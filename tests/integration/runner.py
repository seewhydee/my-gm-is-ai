...
from tests.integration.driver import PlayerDriver
    ...
    *,
    ...
    try:
        for i in range(max_turns):
            if session.is_over:
                break
            # If combat ended but the game isn't over, keep playing (the
            # directive may call for post-combat actions).
            try:
        ...
        result.final_status = session.status_snapshot().to_dict()
