...
Usage:
    python scripts/generate_datapack_manifest.py           # regenerate
    python scripts/generate_datapack_manifest.py --check   # verify sync
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="verify the manifest is in sync with the data pack "
    ...
    if args.check:
        ...
        if current != manifest:
            print(f"{OUT_PATH.relative_to(REPO_ROOT)} is out of sync with "
                  f"mgmai/data/srd_5e/ — rerun {Path(__file__).name}")
            sys.exit(1)
