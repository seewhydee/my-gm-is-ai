...
| `abilities`          | string[] | IDs of [Abilities](corpus.md#abilities) the player currently has available to cast/use — the *prepared* subset for prepared casters, the whole list otherwise.  `CombatAction.ability_id`, briefings, and validation all read this as the castable set |
| `spellbook`          | string[] | Every ability the player knows / has in their spellbook (default `[]` = none).  For prepared casters (cleric, wizard, …), `abilities` is the prepared subset and `spellbook` is the known superset; re-preparation after a long rest swaps `abilities`.  Empty for spontaneous casters and non-casters, in which case `abilities` is the whole list and the prepare-spells menu is hidden.  Invariant when non-empty: `abilities` ⊆ `spellbook` |
